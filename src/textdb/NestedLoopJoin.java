...
public class NestedLoopJoin extends Operator {
	...
	public Tuple next() throws IOException {
		...
		// Use boolean to determine if tupleLeft joins with tupleRight with
		// pred.isEqual(tupleLeft,tupleRight)
		do {
			...
			if (tupleRight == null) {
				input[1].close(); // R is exhausted for current S

				tupleLeft = input[0].next();
				if (tupleLeft == null) {
					return null; // done, exhausted R and S
				}
				input[1].init(); // re-initializes scan of R
				tupleRight = input[1].next();
			}
